...
for num_users in user_values:
    ...
    # Train the model and record NMSE per epoch
    nmse_per_epoch = []
    for epoch in range(1, epochs + 1):
        ...
        print(f"Users: {num_users}, Epoch: {epoch}, NMSE: {nmse:.4f} dB")
    ...
    # Save model
    model.save(f"unet_model_{num_users}_users.h5")

# Save results to Excel
df_results = pd.DataFrame({
    'Epoch': [epoch for result in results for epoch in result['epochs']],
    'User Count': [result['num_users'] for result in results for _ in result['epochs']],
    'NMSE': [nmse for result in results for nmse in result['nmse_values']]
})
df_results.to_excel("nmse_vs_epoch_results.xlsx", index=False)

# Plot NMSE vs. Epoch for all user values
plt.figure(figsize=(10, 6))
for result in results:
    plt.plot(result['epochs'], result['nmse_values'], marker='o', label=f"{result['num_users']} Users")

plt.xlabel('Epochs')
plt.ylabel('NMSE (dB)')
plt.title('NMSE vs. Epoch for Different User Counts')
plt.legend()
plt.grid(True)
plt.savefig("nmse_vs_epoch_plot.png")
plt.show()
